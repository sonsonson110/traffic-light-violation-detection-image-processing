...
license_plate_cascade = cv2.CascadeClassifier(r'src\haarcascade_russian_plate_number.xml')

# Create a list to store unique penalized license plate texts
penalized_texts = []

# Biến hiển thị ảnh demo
show_yellow = True
show_red = True
show_stopline_mask = True
show_edges_processing = True
...
def detect_traffic_light_color(image, rect):
    global show_yellow, show_red # use global
    ...
    if cv2.countNonZero(red_mask) > 0:
        ...
        if (show_red):
            # show_roi_hsv_binarymask(roi, hsv, red_mask, color)
            show_red = False
        ...
        if (show_yellow):
            # show_roi_hsv_binarymask(roi, hsv, yellow_mask, color)
            show_yellow = False
...
def display_random_image(image, title):
    plt.imshow(image)
    plt.title(title)
    plt.show()

def display_masks_line_detection(masks, gray, edges):
    # global show_stopline_mask
    # if (not show_stopline_mask):
    #     return
    # show_stopline_mask = False
    # Plot the roi and binary mask
    fig, axes = plt.subplots(2, 3, figsize=(10, 5))

    for i in range(len(masks)):
        axes[0, i].imshow(masks[i])
        axes[0, i].set_title(f'mask {i}')

    axes[1, 0].imshow(gray, cmap=plt.cm.gray)
    axes[1, 0].set_title("gray")

    axes[1, 1].imshow(edges, cmap=plt.cm.gray)
...
def display_edges_processing(dilated_edges, erode_edges, lines, line_mask):
    # global show_edges_processing
    # if (not show_edges_processing):
    #     return
    # show_edges_processing = False
    # Plot the roi and binary mask
    fig, axes = plt.subplots(1, 3, figsize=(10, 5))

    axes[0].imshow(dilated_edges, cmap=plt.cm.gray)
    axes[0].set_title("dilated_edges")

    axes[1].imshow(erode_edges, cmap=plt.cm.gray)
    axes[1].set_title("erode_edges")

    axes[2].imshow(line_mask, cmap=plt.cm.gray)
    axes[2].set_title("mask_line")
    plt.show()
...
class LineDetector:
    ...
    def detect_white_line(self, frame, color,
        ...
        d_mask1 = cv2.line(mask1.copy(), (0, int(line1(0))), (width, int(line1(width))), (255, 255, 255), 20)
        ...
        d_mask2 = cv2.line(mask2.copy(), (0, int(line2(0))), (width, int(line2(width))), (255, 255, 255), 20)
        ...
        d_mask3 = cv2.line(mask3.copy(), (int(line3(0)), 0), (int(line3(height)), height), (255, 255, 255), 20)
        ...
        # debug
        display_masks_line_detection(
            masks=[d_mask1, d_mask2, d_mask3],
            gray=gray, edges=edges
        )
        ...
        lines = cv2.HoughLinesP(edges, 1, np.pi/180, 150, minLineLength=160, maxLineGap=5)
        # lines = cv2.HoughLinesP(edges, 1, np.pi/180, 10, minLineLength=80, maxLineGap=5)
        ...
        # debug
        # display_edges_processing(dilated_edges, edges, lines, mask)
        ...
        # debug
        # display_random_image(mask_line, 'final_mask')
        # display_random_image(frame, 'frame')
...
def main():
    ...
    # Open the video file
    vid = cv2.VideoCapture(r'src\traffic_video.mp4')
